                                           ...
                                           Ptr<NetDevice> device) const
{
  ...
  if (m_limitsEnabled) {
    ...
    // if (limits == 0)
    // {
    //     NS_FATAL_ERROR ("Limits are enabled, but the selected forwarding strategy does not
    ...
    if (p2p != 0) {
      ...
      // double maxInterestPackets = 1.0  * dataRate.Get ().GetBitRate () / 8.0 / (m_avgDataSize +
      // m_avgInterestSize);
