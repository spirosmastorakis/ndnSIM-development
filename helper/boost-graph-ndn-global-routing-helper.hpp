...
struct WeightCombine
             ...
             property_traits<EdgeWeights>::reference b) const
  {
    if (std::get<0>(a) == 0)
      ...
      return std::make_tuple(std::get<0>(a), std::get<1>(a) + std::get<1>(b), std::get<2>(a) + std::get<2>(b));
  }
};
...
get(const boost::EdgeWeights&, ns3::ndn::GlobalRouter::Incidency& edge)
{
  if (std::get<1>(edge) == 0)
    ...
    // ns3::Ptr<ns3::ndn::Limits> limits = edge.std::get<1> ()->GetObject<ns3::ndn::Limits> ();
    double delay = 0.0;
    // if (limits != 0) // valid limits object
    //   {
    //     delay = limits->GetLinkDelay ();
    //   }
    return property_traits<EdgeWeights>::reference(std::get<1>(edge), static_cast<uint16_t>(std::get<1>(edge)->getMetric()),
                                                   delay); // ns3::Ptr<ns3::ndn::Limits> limits =
                                                           // edge.std::get<1>
                                                           // ()->GetObject<ns3::ndn::Limits> ();
                                                           // double delay = 0.0;
    // if (limits != 0) // valid limits object
    //   {
    //     delay = limits->GetLinkDelay ();
...
get(DistancesMap& map, ns3::Ptr<ns3::ndn::GlobalRouter> key)
{
  ...
  if (i == map.end())
    return std::tuple<std::shared_ptr<nfd::Face>, uint32_t, double>(0,
