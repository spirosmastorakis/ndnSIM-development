...
L3AggregateTracer::OutInterests  (const Interest& interest, const Face& face)
{
  ...
  if (interest.hasWire ())
    {
      m_stats[Ptr<Face> (const_cast<Face*>(&face))].get<1> ().m_outInterests += interest.wireEncode ().size ();
    }
}
...
L3AggregateTracer::InInterests   (const Interest& interest, const Face& face)
{
  ...
  if (interest.hasWire ())
    {
      m_stats[Ptr<Face> (const_cast<Face*>(&face))].get<1> ().m_inInterests += interest.wireEncode ().size ();
    }
}
